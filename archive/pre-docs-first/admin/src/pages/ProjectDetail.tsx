...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    if (!projectId || !selectedPrd) return;
    api.prd(projectId, selectedPrd.filename)
      .then((d) => setPrdContent(d.content))
      .catch((e) => setError(String(e)));
  }, [projectId, selectedPrd]);
  ...
  const runPipeline = async (action: string) => {
    ...
    try {
      const body: Record<string, string> = {
        ...
        prd_path: selectedPrd.path,
      };
      if (action === 'generate') {
        body.prd_id = prdIdFromFilename(selectedPrd.filename);
        body.type = 'all';
      }
  ...
  return (
    <div>
      ...
      <div className="card" data-testid="pipeline-panel">
        ...
        {activeJob && (
          <div style={{ marginTop: '1rem' }} data-testid="pipeline-log">
            ...
            <div className="log-box">{activeJob.log || '（等待日志…）'}</div>
          </div>
        )}
      </div>
      ...
      <div className="card" data-testid="prd-list">
        ...
        {selectedPrd && (
          <details style={{ marginTop: '1rem' }}>
            <summary>预览 PRD 内容</summary>
            <pre className="pre-wrap">{prdContent.slice(0, 8000)}</pre>
          </details>
        )}
      </div>
      ...
      <div className="card" data-testid="report-traceability">
        ...
        {reportContent && (
          <pre className="pre-wrap" style={{ marginTop: '1rem' }}>{reportContent}</pre>
        )}
      </div>
